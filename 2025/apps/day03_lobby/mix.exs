defmodule Lobby.MixProject do
  ...
  def project do
    [
      ...
      escript: [main_module: Lobby.CLI, path: "../../_build/bin/lobby"],
      deps: deps(),
      aliases: aliases()
    ]
  end
  ...
  defp run_escript(_) do
    Mix.shell().cmd("escript ../../_build/bin/lobby -f #{__DIR__}/input/puzzle_input.txt")
  end

  defp run_escript_with_args(_) do
    args = System.argv() |> Enum.join(" ")
    Mix.shell().cmd("escript ../../_build/bin/lobby #{args}")
  end
end
