...
fn main() {
  ...
  let n_flashes = flashes(&filename, &steps);
  println!("1. Amount of flashes after {} steps: {}", steps, n_flashes);

  let steps = all_flash(&filename);
  println!("2. All octopus flash after {} steps.", steps);
}

fn flashes(filename: &String, steps: &usize) -> usize {
...
fn all_flash(filename: &String) -> usize {
  let contents = fs::read_to_string(filename)
    .expect("Couldn't read input file.");
  let lines: Vec<&str> = contents.lines().collect();
