...
fn task_1(c: &mut Criterion) {
  ...
  group.bench_function("Task 1 - Count 1, 4, 7 and 8", |b| {
    b.iter(|| {
      seven_segment_search::get_amount_of_1478(black_box(
...
fn task_2(c: &mut Criterion) {
  let mut group = c.benchmark_group("Day 08");
  group.bench_function("Task 2 - Decode and sum outputs", |b| {
    b.iter(|| {
      seven_segment_search::sum_all_outputs(black_box(
        &INPUT_FILENAME.to_string(),
      ))
    })
  });
