...
fn task_1(c: &mut Criterion) {
  let mut group = c.benchmark_group("Day 00");
  group.bench_function("Task 1 - Rust Template", |b| {
    b.iter(|| rust_template::solution_1(black_box(&INPUT_FILENAME.to_string())))
  });
  group.finish();
}
...
fn task_2(c: &mut Criterion) {
  ...
  group.bench_function("Task 2 - Rust Template", |b| {
    b.iter(|| rust_template::solution_2(black_box(&INPUT_FILENAME.to_string())))
  });
  group.finish();
}
