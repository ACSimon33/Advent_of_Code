...
#[cfg(test)]
mod beacon_scanner_tests {
  use super::{count_beacons, manhatten_distance};
  use pretty_assertions::assert_eq;
  const INPUT_FILENAME_1: &str = "input/example_input.txt";

  #[test]
  fn task_1() {
    assert_eq!(count_beacons(&INPUT_FILENAME_1.to_string()), 79);
  }

  #[test]
  fn task_2() {
    assert_eq!(manhatten_distance(&INPUT_FILENAME_1.to_string()), 3621);
  }
}
